...
public class OutputLayer implements NNLayer{
    ...
    public OutputLayer(int width,ActivationFunction activation)
    {
        this.width=width;
    ...
    public RealMatrix Backward(RealMatrix inflowingResult,double learningRate)
    {
        ...
        RealMatrix m_temp = result.subtract(inflowingResult);
        ...
        for(int i=0;i<r;i++)
        {
            for(int j=0;j<c;j++)
            {
                res = m_temp.getEntry(i, j) * this.localGradient.getEntry(i, j);
